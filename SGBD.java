...
public class SGBD {
    ...
    public void Run() {
        ...
        System.out.println("SGBD démarré. Entrez une commande :");
    ...
    private void ProcessCreateTable(String cmd) {
        try {
            ...
            System.out.println("Table " + name + " créée");
    ...
    // Lit un fichier externe ligne par ligne pour insérer tous ses enregistrements dans une table.
    private void importation(String cmd) throws Exception {
        ...
        if (!partieFichier.startsWith("(") || !partieFichier.endsWith(")")) {
            throw new Exception("Erreur de syntaxe : Le nom du fichier doit être entre parenthèses. Ex: ALLRECORDS(monfichier.csv)");
        }
        
        String nomFichier = partieFichier.substring(1, partieFichier.length() - 1).trim();
        ...
        File f = new File(config.getDbpath(), nomFichier);
        
        if (!f.exists()) throw new Exception("Fichier introuvable : " + f.getAbsolutePath());
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) { 
            ...
            while ((line = br.readLine()) != null) {
                try {
                    ...
                    System.out.println("Erreur ignorée sur la ligne : " + line); 
                }
            }
        }
